...
def put7(
        canvas, image, a, b, alignment="00"
):  # this is the same as put(), but using windows's weird transparency algorithm. ImageRGB+(BackgroundRGB*ImageAlpha).   this assumes that background alpha is 1(fully opaque), i haven't figured out what it does on a transparent background
...
def createtext7(im, x, y, text, fontdirectory, color=(0, 0, 0, 255), buffersize=(3000, 3000), align="00", kerningadjust=0, fit=9999999):
    ...
    # drawntext.show()
    red = ImageMath.eval(
        "convert( int(((i*(255-t)/255+(c*t)/255)*a/255+i*(255-a)/255)*o/255+(i*(255-o))/255) , 'L')", i=ir, t=r, c=c[0], a=a, o=c[3]
        )  # i is the image RGB,  t is the text RGB,  c is the RGB color variable,  a is the text alpha,  o is the alpha color variable
    ...
    alpha = ImageMath.eval(
        "convert( int(((((r+g+b)/3+(255-(r+g+b)/3)*i/255))*t/255+(i*(255-t))/255)*o/255+(i*(255-o))/255) , 'L')", i=ia, r=r, g=g, b=b, t=a, o=c[3]
        )  # i is the image alpha,  r,g,b are RGB values of the text,  t is text alpha,  o is color alpha
...
def CreateXPWindow(
        width, height, captiontext="", active=True, insideimagepath="", erroriconpath="", errortext="", button1="", button2="", button3="", button1style=0, button2style=0, button3style=0
        ):
    if active:
...
def CreateMacAlertDialog(
        width, height, title="", bar=True, icon="", errortext="", subtext="", button1="", button2="", button3="", button1default=False, button2default=False, button3default=False,
        button1style=0, button2style=0, button3style=0
        ):
...
def CreateMacWindow(
        width, height, title="", icon="", errortext="", button1="", button2="", button3="", button1default=False, button2default=False, button3default=False, button1style=0, button2style=0,
        button3style=0
        ):
...
def stretch(size, amount, c):
    result = size - size * (size / (
                size - size / amount))  # this is needed because deform() does the opposite of what you would think it will do, it takes 4 points, and then squishes them into a rectangle.
...
def Create98Window(
        icon="", text="", title="", buttons=[], active=True, closebutton=True, gradient1active=(0, 0, 128), gradient2active=(16, 132, 208), gradient1inactive=(128, 128, 128),
        gradient2inactive=(181, 181, 181)
        ):
    width = 0
    height = 0
    textshift = 0
